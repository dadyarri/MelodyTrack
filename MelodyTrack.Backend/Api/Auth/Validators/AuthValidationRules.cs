...
internal static class AuthValidationRules
{
    ...
    public static void ApplyRequiredPasswordRules<T>(
        this IRuleBuilder<T, string> ruleBuilder,
        string propertyDisplayName,
        string? commonPasswordsPath)
    {
        ruleBuilder
            .NotEmpty()
            .WithMessage($"{propertyDisplayName} не должен быть пустым")
            .MinimumLength(8)
            ...
            .Custom((value, ctx) =>
            {
                ...
                {
                    ctx.AddFailure(ctx.PropertyPath, "Пароль не должен быть частоиспользуемым");
                }
            });
    }

    public static string? ResolveCommonPasswordsPath(string contentRootPath)
    {
