...
[ApiEndpoint(ApiMethod.Post, "/vacation-requests/{id}/decline")]
public sealed class DeclineVacationRequestEndpoint
{
        ...
        CancellationToken ct)
    {
        ...
        var result = await workflow.DeclineAsync(id, request.ExpectedVersion, request.Message, currentUser, ct);
        ...
        var response = await queryService.GetAccessibleAsync(id, currentUser, ct)
            ?? throw new InvalidOperationException("Declined vacation request could not be loaded.");
