    ...
    public CreateCourseRequestValidator()
    {
        ...
        RuleFor(x => x)
            .Custom((request, context) =>
            {
                ...
                {
                    context.AddFailure("Blocks", "Порядок блоков должен быть уникальным.");
                }
                ...
                foreach (var block in blocks)
                {
                    ...
                    {
                        context.AddFailure("Blocks", $"Порядок веток в блоке \"{block.Title}\" должен быть уникальным.");
                    }
                    ...
                    foreach (var branch in branches)
                    {
                        ...
                        {
                            context.AddFailure("Blocks", $"Порядок тем в ветке \"{branch.Title}\" должен быть уникальным.");
                        }
                    }
                }
                ...
                {
                    context.AddFailure("Blocks", $"Ключ темы \"{duplicateKey}\" должен быть уникальным.");
                }
                ...
                foreach (var theme in allThemes)
                {
                    foreach (var dependencyKey in theme.DependencyKeys.Distinct(StringComparer.OrdinalIgnoreCase))
                    {
                        ...
                        {
                            context.AddFailure("Blocks", $"Тема \"{theme.Title}\" ссылается на неизвестную зависимость \"{dependencyKey}\".");
                        }

                        if (string.Equals(theme.Key, dependencyKey, StringComparison.OrdinalIgnoreCase))
                        {
                            context.AddFailure("Blocks", $"Тема \"{theme.Title}\" не может зависеть сама от себя.");
                        }
                    }
                }
            });
