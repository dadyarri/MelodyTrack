...
After a release pull request reaches `master`, monorepo CI verifies both applications and builds the unified `ghcr.io/<owner>/melody-track` production image. The final image contains the ASP.NET runtime, Init, Backend, and the compiled SPA, but no Node.js or nginx runtime. `ReleaseTool` then creates the annotated `v<version>` tag and GitHub Release when the merge is a valid release pull request. Ordinary master changes produce no release.
